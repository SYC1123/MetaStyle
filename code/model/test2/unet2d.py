...
from code.model.test2.layers import *
...
class Unet2D(nn.Module):
    def __init__(self, c=1, n=16, norm='in', num_classes=2):
    ...
    def forward(self, x, mode='meta-train_FM', weights=None):
        # stop_gradient = Tru
        if weights == None:
            x1 = self.convd1(x)
            x2 = self.convd2(x1)
            ...
            y1_pred = conv2d(meta_styled_y, weights['seg1.weight'], weights['seg1.bias'], kernel_size=None, stride=1, padding=0) #+ upsample(y2)
            rec_out = self.reconstruction(meta_styled_y)
        predictions = F.sigmoid(input=meta_styled_y)
        # pred_compact = torch.argmax(predictions, dim=1) # shape [batch, w, h]

        return y1_pred, rec_out
