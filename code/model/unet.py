...
from ..util.meta_style import StyleFeatureBank, compute_statistics, mix_statistics, apply_meta_style
...
class Unet2D(nn.Module):
    def __init__(self, mode='meta-train_FM', c=3, n=16, norm='bn', num_classes=2, activation='relu'):
    ...
    def forward(self, x, weights=None):
        ...
        x5 = self.convd5(x4)
        # 获取特征统计量
        mean, sig = compute_statistics(x5)

        if self.mode == 'meta-train_FM':
            # 元训练特征统计量加入特征bank
            ...
            meta_styled_y = apply_meta_style(x5, mixed_mean, mixed_std)
            # 解码器
            y4 = self.convu4(meta_styled_y, x4)
            y3 = self.convu3(y4, x3)
            y2 = self.convu2(y3, x2)
            y1 = self.convu1(y2, x1)
            seg_out = self.seg1(y1)
